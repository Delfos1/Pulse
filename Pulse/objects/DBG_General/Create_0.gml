...
dbg_button("Import Particle",function(){
	...
	if _p != ""
	{
		var _part = pulse_import_particle(_p,false)	,
				_check = false
		...
		if !_check
		{
			var _inst = instance_create_layer(0,0,layer,DBG_Particle,{particle: _part})
			array_push(particles,_inst)
			p_l = array_length(particles)-1
		}
	}
...
dbg_button("Import System",function(){
	...
	if _p != ""
	{
		var _sys = pulse_import_system(_p,false),
		_check = false
		...
		if !_check
		{
			var _inst = instance_create_layer(0,0,layer,DBG_System,{system: _sys})
			array_push(systems,_inst)
			s_l = array_length(systems)-1
		}
	}
